...
# Função para executar comandos com ou sem sudo conforme necessário
run_command() {
    if [[ $EUID -ne 0 ]]; then
        # Não é root, tenta com sudo primeiro
        if command -v sudo &> /dev/null; then
            sudo "$@"
        else
            # sudo não existe, tenta sem sudo
            "$@"
        fi
    else
        # É root, executa diretamente
        "$@"
    fi
}
